	...
	case 1:
		...
		else
		{
			...
			case FLASH_VALUE_WAKE_RTC:
				...
				// if(Sleep_Mode.bits.b1ForceToSleep_L2 || Sleep_Mode.bits.b1ForceToSleep_L1) {
				// 	if(FLASH_COMPLETE == FlashWriteOneHalfWord(FLASH_ADDR_WAKE_TYPE, FLASH_VALUE_WAKE_OTHER)) {

				// 	}
				// 	else {
				// 		//FLASH?í?ó??????System_ERROR_UserCallback()??????????
